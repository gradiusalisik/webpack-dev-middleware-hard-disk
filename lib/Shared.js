...
module.exports = function Shared(context) {
	var share = {
		...
		setFs: function (compiler) {
			...
			// should not use in-memory file system
			// use first available fs for multi-compilers mode
			context.fs = compiler.compilers ?
	...
	context.compiler.hooks("done", share.compilerDone);
	context.compiler.hooks("invalid", share.compilerInvalid);
	context.compiler.hooks("watch-run", share.compilerInvalid);
	context.compiler.hooks("run", share.compilerInvalid);
